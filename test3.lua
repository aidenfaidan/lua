...
local function GetActualOreModel(hitbox)
    -- Mencari model utama dari ore (mungkin hitbox berada di dalam struktur yang lebih kompleks)
    local model = hitbox.Parent
    if model and model:IsA("Model") then
        return model
    end
    -- Jika tidak ada model, coba naik ke parent
...
local function CreateESP(hitbox)
    ...
    -- Dapatkan model utama untuk highlight
    local oreModel = GetActualOreModel(hitbox)
    
    -- Buat Highlight (warna mengikuti bentuk objek)
    local highlight = Instance.new("Highlight")
    highlight.Name = "OreHighlight"
    highlight.Adornee = oreModel
    highlight.FillColor = color
    highlight.OutlineColor = color
    highlight.FillTransparency = 0.7  -- Lebih transparan agar bentuk terlihat
    highlight.OutlineTransparency = 0.3
    highlight.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop
    highlight.Parent = oreModel
    
    -- Billboard untuk info
    ...
    for hitbox, data in pairs(ESP_ByHitbox) do
        if not hitbox or not hitbox.Parent then 
            ...
            if not visible then 
                ...
                -- Update highlight position jika diperlukan
