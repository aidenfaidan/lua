...
local function BuildTelegramMessage(fishInfo, fishId, fishRarity, weight)
    ...
    local message = "```\n"
    message = message .. "AUTO FISHING HUB - TELEGRAM HOOK\n"
    message = message .. "DEVELOPER: HyRexxyy x GPT\n"
    message = message .. "========================================\n"
    message = message .. "\n"
    ...
    message = message .. "     FISH COUNT: " .. tostring(fishCount) .. "\n"
    message = message .. "\n"
...
HookTab:CreateParagraph({ 
    Title = "📋 Token Info", 
    Content = "Bot token is pre-configured in the script. Make sure Chat ID is filled correctly." 
})
...
HookTab:CreateButton({ 
    Name = "Test LEGENDARY Notification", 
    Callback = function()
        ...
        local msg = BuildTelegramMessage(testFish, 999, "LEGENDARY", 3.45)
        local ok = SendTelegram(msg)

        Rayfield:Notify({
            Title = ok and "✅ Test Sent" or "❌ Test Failed",
            Content = ok and "Legendary test notification sent!" or "Failed to send notification",
            Duration = 4
        })
    end 
})
...
HookTab:CreateButton({ 
    ...
    Callback = function()
        ...
        local msg = BuildTelegramMessage(testFish, 888, "EPIC", 2.15)
        local ok = SendTelegram(msg)

        Rayfield:Notify({
            Title = ok and "✅ Test Sent" or "❌ Test Failed",
            Content = ok and "Epic test notification sent!" or "Failed to send notification",
            Duration = 4
        })
    end 
})
...
print("🔔 Hook System: Telegram Notifications with Rarity Filter")  
