...
-- LOAD DATABASE FOR PC - SIMPLIFIED PATH
local function LoadDatabase()
    local path = "FULL_ITEM_DATA.json" -- Current directory only for PC
...
-- ================= AUTO FISHING V2 (FULL AUTO NO TAP) =================

local function AutoFishingV2()
    task.spawn(function()
        print("[AutoFishingV2] Started - Full Auto Mode Enabled")

        -- Pastikan karakter dan alat siap
        ...
        -- Equip hanya sekali di awal
        if EquipTool then
            print("[AutoFishingV2] Equipping rod once...")
            pcall(function()
                EquipTool:FireServer(1)
            end)
            task.wait(0.5)
        ...
        -- Aktifkan auto-fishing sistem bawaan game
        pcall(function()
            UpdateAutoFishing:InvokeServer(true)
        end)

        -- Hook StartMini agar otomatis melempar pancing
        ...
        if mt then
            ...
            mt.__namecall = newcclosure(function(self, ...)
                ...
                if method == "InvokeServer" and self == StartMini and Config.AutoFishingV2 then
                    -- Otomatis lempar dengan koordinat fixed
        ...
        -- Loop auto fishing penuh tanpa tap
        while Config.AutoFishingV2 do
            local success, err = pcall(function()
                ...
                print("[AutoFishingV2] Fish caught automatically!")
                task.wait(0.5)
            end)

            if not success then
                warn("[AutoFishingV2] Error: " .. tostring(err))
                task.wait(1)
            end
        end

        -- Matikan sistem auto game saat user stop
        pcall(function()
...
-- ================= AUTO FISHING STABLE (NO RE-EQUIP, FIXED 1.5s DELAY) =================

function AutoFishingStable()
    task.spawn(function()
        print("[AutoFishingStable] Started (Continuous Mode, Delay: 1.5s)")
        ...
        -- Equip hanya sekali di awal
        if EquipTool then
            print("[AutoFishingStable] Equipping rod once...")
            pcall(function()
                EquipTool:FireServer(1)
            end)
            task.wait(0.5)
        ...
        while Config.AutoFishingStable do
            local success, err = pcall(function()
                ...
                -- Tunggu stabil 1 detik sebelum tangkap
                task.wait(1.0)
                ...
                print("[AutoFishingStable] Fish caught (Continuous, 1.5s Cycle)")
                task.wait(0.5)
            end)
